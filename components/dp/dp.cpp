...
void DP::loop() {
  // No need to read data in the loop function; all UART reading logic is in update
}
...
void DP::readData() {
  ...
  *hexPtr = '\0';
  ...
  if (validStartByte && (hexPtr - hexString) == 12 && hexString[10] == '0' && hexString[11] == '0') {
    valid6thByte = true;
...
void DP::publishSensorStates() {
  // Additional state publishing if necessary
}
